...
public class CodeGen {
	...
	void M (Program p, String filename) throws IOException {
		...
		JasminFile assem_out = new JasminFile(jfile + ".jas");

        //Mudando o inicio do arquivo.j
	...
	//Modifications
	private void printvarmenu(Function f, JasminFile jfile) throws IOException {
		jfile.writeln(".var");
		//System.out.println(".var");
		String[] variaveis = new String[10];
		Block b = f.body;
		int i=0;
		for (Statement member : b.members) {
			try {

				Assignment assignment = (Assignment) member;
				if(!testar_se_tem_rep(variaveis, String.valueOf(assignment.target.id))) {
					System.out.println("N é igual");
					jfile.writeln(" " + assignment.target.id);
					variaveis[i] = String.valueOf(assignment.target.id);
					i++;
				}else {
		...
		jfile.writeln(".end-var");
		//System.out.println(".end-var");
	}

	private boolean testar_se_tem_rep(String array[], String name_var){
		for(String nomevar : array){
			if(nomevar != null){
				if(nomevar.equals(name_var)){
					return true;
				}
			}
		}
    ...
    void M (Assignment a, SymbolTable symtable, JasminFile jfile) throws IOException {
		...
		//Modifications do by Nathan
		//jfile.writeln(store + " " + symtable.getIndex((Variable) a.target));
		jfile.writeln(store + " " + a.target.id);
	...
	*/
	...
	if (op.val.equals(Operator.INT_LT)) {
		jfile.write("\tif_icmpeq LOOPEXIT"+(branch_cnt-1));
		...
		jfile.writeln("if_icmpeq TRUE"+(branch_cnt-1));
		//jfile.write_relop_body(branch_cnt);
		branch_cnt++;
		return;
